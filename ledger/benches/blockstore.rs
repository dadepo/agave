...
#[bench]
fn bench_get_data_shreds_for_slot(bench: &mut Bencher) {
    let slot = 10;
    let entries = make_slot_entries_with_transactions(100);
    let shreds = entries_to_test_shreds(
        &entries,
    ...
    bench.iter(move || {
        blockstore.get_data_shreds_for_slot(slot + 2, 0).unwrap();
    });
}
...
#[bench]
fn bench_get_slot_entries_with_shred_info(bench: &mut Bencher) {
    let slot = 10;
    let entries = make_slot_entries_with_transactions(100);
    let shreds = entries_to_test_shreds(
        &entries,
    ...
    bench.iter(move || {
        blockstore.get_slot_entries_with_shred_info(slot + 2, 0, true).unwrap();
    });
}